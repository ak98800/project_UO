from ._anvil_designer import VueMontanteViewTemplate
from anvil import *
import anvil.users
import anvil.server
...
class VueMontanteView(VueMontanteViewTemplate):
  def __init__(self, dossier, societe_point_depart=None, societe=None, **properties):
    self.init_components(**properties)

    self.dossier = dossier

    # compatible : soit on reçoit societe_point_depart (string), soit societe (dict)
  ...
  def _afficher_organigramme(self):
    if not self.societe_point_depart:
      alert("Aucune société de départ.")
      self._nodes, self._edges = [], []
      return

    dossier_name = self.dossier['name']

    # ✅ appel serveur dédié (comme descendante)
    relations = anvil.server.call(
      'get_relations_montantes',
      dossier_name,
      self.societe_point_depart
    )
